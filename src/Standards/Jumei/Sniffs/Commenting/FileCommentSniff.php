...
class Jumei_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    protected function processTags($commentStart, $commentEnd)
    {
        ...
        foreach ($tags as $tag => $orderText) {
            ...
            if (in_array($tag, $foundTags) === false && in_array($tag, $requiredTag)) {
    ...
    /**
     * Author tag must be 'Xxx Yyyy<yyyx@jumei.com>'.
     *
     * @param int $errorPos The line number where the error occurs.
     *
     * @return void
    ...
    protected function processAuthors($errorPos)
    {
        ...
        if (empty($authors) === false) {
            ...
            } else if ( ! preg_match('%^.*\<[a-z0-9]+\@jumei\.com\>$%', $content) ) {
                $error = 'Expected "Xxx Yyy<yyyx@jumei.com>" for author tag';
